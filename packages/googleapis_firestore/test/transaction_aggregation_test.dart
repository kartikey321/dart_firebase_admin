...
void main() {
  group('Transaction - Aggregation Queries', () {
    ...
    test('getAggregation() with count works', () async {
      ...
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(collection.count());
      });
    ...
    test('getAggregation() with sum works', () async {
      await collection.add({'price': 10});
      await collection.add({'price': 20});
      await collection.add({'price': 30});

      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(collection.sum('price'));
      });

      expect(snapshot.getSum('price'), 60);
    });

    test('getAggregation() with average works', () async {
      await collection.add({'score': 80});
      await collection.add({'score': 90});
      await collection.add({'score': 100});

      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(collection.average('score'));
      });

      expect(snapshot.getAverage('score'), 90.0);
    });

    test('getAggregation() with multiple aggregations works', () async {
      ...
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(aggregation);
      });
    ...
    test('getAggregation() throws on read-after-write', () async {
    ...
    test('getAggregation() works in read-only transaction', () async {
    ...
    test('getAggregation() works with filtered queries', () async {
      ...
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(query.sum('price'));
      });

      expect(snapshot.getSum('price'), 40);
    });

    test('getAggregation() provides consistent snapshot', () async {
      await collection.add({'value': 10});
      await collection.add({'value': 20});
      final results = await firestore.runTransaction((transaction) async {
        final agg1 = await transaction.getAggregation(collection.count());
        final agg2 = await transaction.getAggregation(collection.sum('value'));
    ...
    test('getAggregation() works with converter', () async {
      ...
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(collection.sum('value'));
      });

      expect(snapshot.getSum('value'), 30);
    });

    test('getAggregation() works with collection groups', () async {
      ...
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(
    ...
    test('multiple getAggregation() calls in same transaction', () async {
      await collection.add({'value': 10, 'category': 'A'});
      await collection.add({'value': 20, 'category': 'B'});
      await collection.add({'value': 30, 'category': 'A'});

      final results = await firestore.runTransaction((transaction) async {
        final totalCount = await transaction.getAggregation(collection.count());
        final categoryA = await transaction.getAggregation(
          collection.where('category', WhereFilter.equal, 'A').count(),
        );
        final totalSum = await transaction.getAggregation(
    ...
    test('getAggregation() mixed with getQuery() in transaction', () async {
      ...
      final result = await firestore.runTransaction((transaction) async {
        final aggSnapshot = await transaction.getAggregation(
    ...
    test('getAggregation() with empty results', () async {
      final snapshot = await firestore.runTransaction((transaction) async {
        return transaction.getAggregation(
    ...
    test('getAggregation() with complex filter', () async {
    ...
    test('getAggregation() mixed with get() in transaction', () async {
